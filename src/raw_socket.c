...
dtn_socket_result_t dtn_init_raw_socket(void) {
    ...
    for (int i = 0; i < dtn_config.interface_count; i++) {
        ...
        if (raw_socket < 0) {
            DTN_ERROR("Failed to create AF_INET6 raw socket for interface %s: errno=%d (%s)", ifname, errno, strerror(errno));
        ...
        if (setsockopt(raw_socket, IPPROTO_IPV6, IPV6_HDRINCL, &one, sizeof(one)) < 0) {
            DTN_ERROR("Failed to set IPV6_HDRINCL for interface %s: errno=%d (%s)", ifname, errno, strerror(errno));
            close(raw_socket);
        ...
        if (setsockopt(raw_socket, SOL_SOCKET, SO_BINDTODEVICE, ifname, strlen(ifname) + 1) < 0) {
            DTN_ERROR("Failed to SO_BINDTODEVICE to %s: errno=%d (%s)", ifname, errno, strerror(errno));
            close(raw_socket);
        ...
        if (setsockopt(raw_socket, SOL_SOCKET, SO_MARK, &mark, sizeof(mark)) < 0) {
            DTN_ERROR("Failed to set SO_MARK for interface %s: errno=%d (%s)", ifname, errno, strerror(errno));
            close(raw_socket);
            return DTN_SOCKET_ERR_SOCKOPT;
        }
    ...
    for (int i = 0; i < dtn_config.interface_count; i++) {
        DTN_INFO("Raw Socket: Initialized socket for interface %s: fd=%d", dtn_config.interfaces[i].name, dtn_config.interfaces[i].socket);
    }
...
dtn_socket_result_t dtn_init_raw_socket(void) {
    ...
    for (int i = 0; i < dtn_config.interface_count; i++) {
        DTN_INFO("Raw Socket: Initialized raw socket with name %s: socket %d, index %d", dtn_config.interfaces[i].name,
                 dtn_config.interfaces[i].socket, dtn_config.interfaces[i].socket_index);
    }
...
dtn_socket_result_t dtn_raw_socket_send(struct pbuf* p) {
    ...
    for (int i = 0; i < dtn_config.interface_count; i++) {
        ...
        for (int j = 0; j < iface->dtn_address_count; j++) {
            if (ip6addr_aton(iface->dtn_addresses[j], &candidate) && ip6_addr_zoneless_eq(&dest_addr, &candidate)) {
                interface_to_use = i;
                break;
            }
        }
        ...
        for (int j = 0; j < iface->address_count; j++) {
            if (ip6addr_aton(iface->addresses[j], &candidate) && ip6_addr_zoneless_eq(&dest_addr, &candidate)) {
                interface_to_use = i;
                break;
            }
        }
    ...
    DTN_INFO("Sending packet to dest: %s using interface %s (fd=%d)", dest_str, iface->name, iface->socket);

    return dtn_raw_socket_send_via_interface(p, iface);
}

#ifdef USE_AF_INET6
dtn_socket_result_t dtn_raw_socket_send_via_interface(struct pbuf* p, const DtnInterface* dtn_interface) {
    if (p->tot_len > 2048) {
    ...
    if (sent_bytes < 0) {
        DTN_ERROR("Failed to send packet via interface %s: errno=%d (%s)", dtn_interface->name, errno, strerror(errno));
        return DTN_SOCKET_ERR_SEND;
    } else if ((uint16_t)sent_bytes != p->tot_len) {
        DTN_WARN("Sent only %d of %d bytes on interface %s", sent_bytes, p->tot_len, dtn_interface->name);
...
static int parse_mac(const char* mac_str, uint8_t mac[6]) {
    return sscanf(mac_str, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) == 6 ? 0 : -1;
}
...
dtn_socket_result_t dtn_raw_socket_send_via_interface(struct pbuf* p, const DtnInterface* dtn_interface) {
    ...
    int sent_bytes = sendto(dtn_interface->socket, buf, frame_len, 0, (struct sockaddr*)&sll, sizeof(sll));

    if (sent_bytes < 0) {
        DTN_ERROR("Failed to send frame via interface %s: errno=%d (%s)", dtn_interface->name, errno, strerror(errno));
        return DTN_SOCKET_ERR_SEND;
    } else if (sent_bytes != frame_len) {
        DTN_WARN("Sent only %d of %d bytes on interface %s", sent_bytes, frame_len, dtn_interface->name);
        return DTN_SOCKET_ERR_PARTIAL;
    }
