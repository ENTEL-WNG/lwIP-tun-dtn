// dtn_routing.c: Implementation of contact-based routing with time-variant contact management for
// DTN networks Copyright (C) 2025 Michael Karpov
//
    ...
    while (contact != NULL) {
        ...
        if (ip6_addr_cmp(&contact_addr_nozone, &dest_addr_nozone)) {
            ...
            if (current_time >= contact->start_time_ms && current_time <= contact->end_time_ms) {
                ...
                DTN_INFO(
                    "DTN Routing: Contact AVAILABLE for DTN destination %s. Providing next hop.",
                    dest_addr_str);
                ...
                char dest_addr_str[IP6ADDR_STRLEN_MAX];
                ip6addr_ntoa_r(dest_ip, dest_addr_str, sizeof(dest_addr_str));
                DTN_INFO("DTN Routing: Contact EXISTS but NOT ACTIVE for DTN destination %s.",
                         dest_addr_str);
                ip6_addr_set_any(next_hop_ip);
                return 0;
            }
