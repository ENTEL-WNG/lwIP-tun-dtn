...
# 1. Dynamic Interface Renaming
# Strips the @suffix (e.g., eth0@if84 -> eth0) and renames to enp0s8/9
...
if [ -z "$OLD_INT1" ] || [ -z "$OLD_INT2" ]; then
    echo "Error: Could not find two ethernet interfaces."
    exit 1
fi
...
echo "Configuring $NAME static IPv6 addresses and routes..."
...
echo "Applying Node 2 Policy Based Routing (Table 100)..."
