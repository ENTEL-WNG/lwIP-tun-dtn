...
// TLV option type embedded in the IPv6 Hop-by-Hop extension header.
// High bits 00 → kernel skips unknown option without error.
#define CUSTODY_OPTION_TYPE 0x1E  // TLV option: custodian IPv6 address (len=16)

// Total HBH extension header length in bytes (must be a multiple of 8):
//   2  fixed header  (next_header + hdr_ext_len)
//  18  custody TLV  (type + len + 16-byte addr)
//   4  Pad1 × 4
//  ──
//  24
